...
func getConfig(projectName string) {
	...
	viper.AddConfigPath(fmt.Sprintf("/$HOME/GoProjects/go-practise/config/%s", projectName)) // path to look for the config file in
...
func GetMysqlConnectingString() string {
	...
	return fmt.Sprintf("%s:%s@tcp(%s:3307)/%s?charset=%s&parseTime=true", usr, pwd, host, db, charset)
}
